...
import { applyThemeMode } from '../ui/theme.js';
...
export async function renderSettings() {
      ...
      <div class="settings-row">
        ...
        <div class="select-wrap">
          ...
          <select class="input" id="uiLangSelect">
            <option value="es" ${(s.uiLang === 'es' || !s.uiLang) ? 'selected' : ''}>Español (Latinoamérica)</option>
            <option value="en" ${s.uiLang === 'en' ? 'selected' : ''}>English (Native)</option>
            <option value="ru" ${s.uiLang === 'ru' ? 'selected' : ''}>Русский</option>
            <option value="ja" ${s.uiLang === 'ja' ? 'selected' : ''}>日本語</option>
            <option value="zh" ${s.uiLang === 'zh' ? 'selected' : ''}>简体中文</option>
          </select>
        </div>
      </div>
  ...
  $('#themeModeSelect')?.addEventListener('change', async (ev) => {
    const mode = ev.target.value;
    applyThemeMode(mode);
    state.settings.themeMode = mode;
    await window.api.settings.set('themeMode', mode);
  });
