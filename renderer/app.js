...
// Mod Assistant account in the title bar (InsForge backend).
function paintAccount() {
  ...
  $('#tbUpgradeBtn')?.addEventListener('click', () => {
    showCheckoutModal({
      onSuccess: async () => {
        const st = await window.api.auth.status();
        if (st.user) state.currentUser = st.user;
        paintAccount();
        render();
      },
    });
  ...
  $('#tbUserBtn')?.addEventListener('click', () => {
...
(async function boot() {
  ...
  const allowedLangs = ['es', 'en', 'ru', 'ja', 'zh'];
  window.I18N_LANG = allowedLangs.includes(cfg.uiLang) ? cfg.uiLang : (allowedLangs.includes(localStorage.getItem('uiLang')) ? localStorage.getItem('uiLang') : 'es');
  ...
  // require user authentication to use the app
