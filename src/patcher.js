...
/**
 * Remove lines injected by foreign patchers (e.g. Dota2SkinChanger, DotaModdingCommunityMods)
 * from branchspecific text so they do not conflict with Mod Assistant.
 */
...
function apply({ gamePath, folder, backupDir }) {
  ...
  const branchBuf = Buffer.from(patchedBranch(cleanForeignText(branchOrig), block), 'latin1');
...
/**
 * Remove foreign patcher lines (e.g. Dota2SkinChanger) from gameinfo_branchspecific.gi
 * and re-sign signatures if present, so Mod Assistant can operate without conflict.
 */
function cleanForeign({ gamePath, folder }) {
  ...
  const cleaned = cleanForeignText(raw);
  if (cleaned !== raw) {
    const branchBuf = Buffer.from(cleaned, 'latin1');
    writeAtomic(p.branch, branchBuf);
