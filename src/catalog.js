...
// Walk every list of mods in a mods.json, whatever shape its category is in: a plain array,
// or a group list for the categories that are sorted by hero.
function eachList(modsData, fn) {
  for (const category of Object.values(modsData || {})) {
    if (!category) continue;
    const lists = Array.isArray(category)
      ? [category]
      : Array.isArray(category.groups)
        ? category.groups.map((g) => (g && g.mods) || [])
        : [category.mods || []];
    for (const list of lists) if (Array.isArray(list)) fn(list);
  }
}

function eachMod(modsData, fn) {
  eachList(modsData, (list) => {
    for (const mod of list) if (mod && typeof mod === 'object') fn(mod);
  });
}

// What makes two entries the same download: the name on the card and the file behind it (a
// mod with looks has a file per look instead of one of its own).
function repeatKey(mod) {
  const file = mod.file != null
    ? String(mod.file)
    : (Array.isArray(mod.styles) ? mod.styles.map((s) => (s && s.file) || '') : '');
  return JSON.stringify([mod.name, file]);
}

/* The catalog carries the odd entry twice: 'ReZero Respawn Sound' is sounds[0] and sounds[12],
 * byte for byte, and a list drawn as it arrives puts two cards on screen with one key between
 * them, sharing a star, a badge and a place in the install list. A repeat is dropped only
 * where the author cannot have meant two things - same name and same file in the same list,
 * a category or one hero's group - and the first copy stays. Two names on one file are left
 * alone ('GLaDOS' and 'Ru GLaDOS' are one pak upstream), and so is one mod filed under two
 * heroes, which is a group filter each of them should answer to. */
function dropRepeats(list) {
  const seen = new Set();
  let keep = 0;
  for (const mod of list) {
    const key = mod && typeof mod === 'object' ? repeatKey(mod) : null;
...
function normalizeCatalog(mods) {
  const data = mods && mods.modsData;
  eachList(data, dropRepeats);
  eachMod(data, (mod) => {
